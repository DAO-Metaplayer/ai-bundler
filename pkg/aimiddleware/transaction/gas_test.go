...
import (
	...
	"github.com/DAO-Metaplayer/ai-bundler/internal/testutils"
	"github.com/DAO-Metaplayer/ai-bundler/pkg/aiop"
	"github.com/ethereum/go-ethereum/common"
)

// TestSuggestMeanGasTipCapForNormalLoad simulates a scenario of normal network load. In this case the average
// tip from aiOps is assumed to not be above the suggested tip. In which case the suggested tip is the
// optimal choice.
func TestSuggestMeanGasTipCapForNormalLoad(t *testing.T) {
	...
	batch := []*aiop.AiOperation{op1, op2}
...
// from aiOps is assumed to be above the suggested tip (i.e. aiOps want to be included quickly). In which
// case the average tip from aiOps is the optimal choice.
...
// gas fee cap from aiOps is assumed to not be above the recommended max fee. In which case the recommended
...
// fee cap from aiOps is assumed to be above the recommended max fee. In which case the average gas fee cap
// from aiOps is the optimal choice.
...
// gas fee cap from aiOps is assumed to not be above the given gas price. In which case the given gas price
...
// fee cap from aiOps is assumed to be above the given gas price. In which case the average gas fee cap from
// aiOps is the optimal choice.
